...
static void mount(int w, int h, uint32_t now) {
  (void)w;(void)h;(void)now;calls=last=0;
}
static void gesture(int kind, uint32_t now) {
  calls++; last=kind; gesture_time=now;
  origin_x=dte_gesture_x();origin_y=dte_gesture_y();
}
static int render(const struct dte_snapshot *s, uint32_t now, uint16_t *p) {
  (void)s;(void)now;(void)p;return 0;
}
const struct dte_theme dte_selected_theme={DTE_ABI_VERSION,"test",mount,gesture,render};
int main(void) {
  dte_init(280,240);
  assert(dte_last_status()==DTE_STATUS_OK);
  assert(dte_active_abi_version()==DTE_ABI_VERSION_V1);
