(async()=>{
  ...
  function hardwareProfile(){const preset=model.profiles[$('hardware-profile').value];if(preset?.unlimited)return preset;return {...preset,fps:+$('target-fps').value,spiMHz:+$('spi-clock').value,cpuScale:+$('cpu-scale').value,transport:$('transport').value,unlimited:false};}
  function dirtyRows(w,h){const count=Math.ceil(h/16),ptr=api.dtr_dirty_tiles?.();if(!ptr)return Array(count).fill((1<<Math.ceil(w/16))-1);return Array.from(new Uint32Array(api.memory.buffer,ptr,count));}
  function updateScreenMask(w,h){if($('screen-mask').value!=='rounded-169'){delete screenShell.dataset.mask;return;}screenShell.dataset.mask='rounded-169';screenShell.style.setProperty('--mask-rx',`${43*100/w}%`);screenShell.style.setProperty('--mask-ry',`${43*100/h}%`);}
  function paintPhysicalMask(w,h){if($('screen-mask').value!=='rounded-169')return;const radius=Math.min(43,Math.floor(Math.min(w,h)/2));ctx.save();ctx.fillStyle='#050709';ctx.beginPath();ctx.rect(0,0,w,h);ctx.roundRect(0,0,w,h,radius);ctx.fill('evenodd');ctx.restore();}
  function paint(){const w=api.dte_width(),h=api.dte_height();if(canvas.width!==w||canvas.height!==h){canvas.width=w;canvas.height=h;}updateScreenMask(w,h);const src=new Uint16Array(api.memory.buffer,api.dte_pixels(),w*h),im=ctx.createImageData(w,h);for(let i=0;i<src.length;i++){const p=src[i];im.data[i*4]=(p>>11)*255/31;im.data[i*4+1]=((p>>5)&63)*255/63;im.data[i*4+2]=(p&31)*255/31;im.data[i*4+3]=255;}ctx.putImageData(im,0,0);paintPhysicalMask(w,h);$('trace').textContent=`time ${time.toFixed(1)} ms\nframe ${w}×${h} · RGB565\nhash ${api.dte_hash().toString(16)} · simulation`;}
  function renderFrame(count=true){const w=api.dte_width(),h=api.dte_height(),profile=hardwareProfile();api.dte_preview_set_strip_pixels?.(profile.unlimited?w*h:(profile.stripPixels||4480));api.dte_preview_set_filter?.(+$('frame-filter').value);api.dte_preview_set_filter_corner_radius?.(+$('filter-radius').value);const started=performance.now();api.dte_render(Math.round(time));lastWasmMs=performance.now()-started;const rows=dirtyRows(w,h);lastTransfer=profile.transport==='abi'&&api.dte_preview_transfer_bytes?model.actualStats(rows,w,h,api.dte_preview_transfer_bytes(),api.dte_preview_dirty_rects(),api.dte_preview_draw_calls()):model.dirtyStats(rows,w,h,profile.transport);lastEstimate=model.estimate(lastWasmMs,profile,lastTransfer);hardwareReadyAt=time+lastEstimate.totalMs;if(count)presentedCount++;paint();updateMetrics();}
  function formatBytes(value){if(value<1024)return `${value} B`;if(value<1024*1024)return `${(value/1024).toFixed(1)} KiB`;return `${(value/1024/1024).toFixed(2)} MiB`;}
  ...
  for(const id of ['wpm','layer','endpoint'])$(id).oninput=()=>{stopDemo();$('wpm-value').value=$('wpm').value;state();renderFrame(false);};
  for(const prefix of ['dongle','split0','split1'])$(prefix+'-battery').oninput=()=>{stopDemo();const v=+$(prefix+'-battery').value;$(prefix+'-value').textContent=v<0?'--':v+'%';state();renderFrame(false);};
  $('battery-count').onchange=()=>{stopDemo();state();renderFrame(false);};$('mods').onclick=e=>{const bit=+e.target.dataset.mod;if(!bit)return;stopDemo();mods^=bit;e.target.classList.toggle('active');state();renderFrame(false);};
  ...
  let contact=null,hold=null;function pointerPosition(e){const r=canvas.getBoundingClientRect();return{x:Math.round((e.clientX-r.left)*canvas.width/r.width),y:Math.round((e.clientY-r.top)*canvas.height/r.height)};}function pointer(e,down){const p=pointerPosition(e),previewX=contact?2*contact.x-p.x:p.x;api.dte_touch(previewX,p.y,down,Math.round(time));renderFrame(false);}
  canvas.onpointerdown=e=>{stopDemo();wakeAnimation();canvas.setPointerCapture(e.pointerId);const p=pointerPosition(e);contact={time,x:p.x};pointer(e,1);hold=setTimeout(()=>{if(contact){time=Math.max(time,contact.time+600);renderFrame(false);syncInteraction();}},600);};canvas.onpointermove=e=>{if(contact)pointer(e,1);};canvas.onpointerup=e=>{clearTimeout(hold);if(!contact)return;pointer(e,0);syncInteraction();contact=null;statusMode='gesture';localizeDynamic();};canvas.onpointercancel=()=>{clearTimeout(hold);contact=null;api.dte_touch_cancel();};
