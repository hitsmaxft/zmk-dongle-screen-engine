...
(()=>{
  const profiles={
    ...
    nrf52840:{fps:24,spiMHz:32,cpuScale:260,transport:'bands',stripPixels:4480,sramBytes:256*1024,flashBytes:1024*1024,unlimited:false},
    custom:{fps:24,spiMHz:16,cpuScale:80,transport:'bands',stripPixels:4480,unlimited:false},
  };
  function dirtyStats(rows,width,height,mode){
    ...
    const result=(tiles,bytes,rects)=>({tiles,bytes,rects,fullBytes,dirtyPercent:fullBytes?bytes*100/fullBytes:0});
  ...
  function estimate(wasmMs,profile,transfer){
    ...
    const transferMs=payloadTransferMs+transfer.rects*.04;
