...
def verify_abi(api,wasm):
    ...
    api.dte_init(280,240);assert api.dte_active_abi_version()==0x0102
    pixels=(C.c_uint16*(280*240))()
    canvas=Canvas(0x0102,C.sizeof(Canvas),280,240,0,0,280,240,280,1,0,C.sizeof(pixels),pixels)
    ...
    snapshot.contents.abi_version=0x0102;snapshot.contents.struct_size=96
    ...
    short=Frame(0x0102,4);assert api.dte_frame(777,C.byref(short))==-3
    wrong=Frame(0x0103,C.sizeof(Frame));wrong_abi=api.dte_frame(777,C.byref(wrong));assert wrong_abi==-2
    frame_bytes=C.create_string_buffer(164);frame=C.cast(frame_bytes,C.POINTER(Frame))
    frame.contents.abi_version=0x0102;frame.contents.struct_size=160
    ...
    js="""const fs=require('fs');(async()=>{const {instance}=await WebAssembly.instantiate(fs.readFileSync(process.argv[1]));const e=instance.exports,m=e.memory;e.dte_init(280,240);const old=m.buffer.byteLength;m.grow(3);const v=new DataView(m.buffer),sp=old,fp=old+128,cp=old+320,pix=old+512;new Uint8Array(m.buffer,old,196608).fill(0);v.setUint16(cp,0x0102,true);v.setUint16(cp+2,28,true);for(const [o,n] of [[4,280],[6,240],[8,0],[10,0],[12,280],[14,240],[16,280]])v.setUint16(cp+o,n,true);v.setUint8(cp+18,1);v.setUint32(cp+20,134400,true);v.setUint32(cp+24,pix,true);const before=e.dte_draw(777,cp);v.setUint16(sp,0x0102,true);v.setUint16(sp+2,96,true);v.setUint32(sp+8,2,true);v.setInt32(sp+20,123,true);v.setUint32(sp+96,0xa5a55a5a,true);const snapshot=e.dte_set_snapshot(sp),snapshotCanary=v.getUint32(sp+96,true)===0xa5a55a5a;v.setUint16(fp,0x0102,true);v.setUint16(fp+2,4,true);const short=e.dte_frame(777,fp);v.setUint16(fp,0x0103,true);v.setUint16(fp+2,160,true);const wrongAbi=e.dte_frame(777,fp);v.setUint16(fp,0x0102,true);v.setUint16(fp+2,160,true);v.setUint32(fp+160,0x5aa5a55a,true);const frame=e.dte_frame(777,fp),frameCanary=v.getUint32(fp+160,true)===0x5aa5a55a,count=v.getUint16(fp+12,true);let bounds=count>0&&count<=18;for(let i=0;i<count;i++){const o=fp+16+i*8,x=v.getInt16(o,true),y=v.getInt16(o+2,true),w=v.getUint16(o+4,true),h=v.getUint16(o+6,true);bounds=bounds&&w>0&&h>0&&x>=0&&y>=0&&x+w<=280&&y+h<=240;}const x=v.getInt16(fp+16,true),y=v.getInt16(fp+18,true),w=v.getUint16(fp+20,true),h=v.getUint16(fp+22,true);for(const [o,n] of [[8,x],[10,y],[12,w],[14,h],[16,w]])v.setUint16(cp+o,n,true);v.setUint32(cp+20,w*h*2,true);const draw=e.dte_draw(777,cp),wrongTime=e.dte_draw(778,cp);v.setUint32(cp+20,2,true);const small=e.dte_draw(777,cp);process.stdout.write(JSON.stringify({abi:e.dte_active_abi_version(),before,snapshot,snapshot_canary:snapshotCanary,short,wrong_abi:wrongAbi,frame,count,frame_canary:frameCanary,bounds,draw,wrong_time:wrongTime,small}));})().catch(e=>{console.error(e);process.exit(1)});"""
    result=json.loads(subprocess.check_output(['node','-e',js,str(wasm)]))
    expected={'abi':0x0102,'before':-8,'snapshot':0,'snapshot_canary':snapshot_canary,'short':-3,'wrong_abi':wrong_abi,'frame':0,'count':frame.dirty_count,'frame_canary':frame_canary,'bounds':bounds,'draw':0,'wrong_time':wrong_time,'small':-9}
...
def verify(output,api_only=False):
    ...
    if api_only:
        result={'abi_1_2_native_wasm':abi_probe}
    ...
    result={'abi_1_2_native_wasm':abi_probe,'native_wasm_equal_frames':len(actual),'viewports':3,'long_press_no_tap':True,'touch_active_lifecycle':True,'deterministic_repeat':True,'static_dither_no_shimmer':static}
