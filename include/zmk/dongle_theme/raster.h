...
#include <stddef.h>
#include <stdint.h>
/* Serialized single-surface raster services shared by all compile-time theme variants. */
struct dtr_clip_rect { int left,top,right,bottom; };
...
void dtr_arc(int cx,int cy,int inner,int outer,int first,int last,int r,int g,int b,int alpha);
void dtr_arc_f(int cx,int cy,float inner,float outer,int first,int last,int r,int g,int b,int alpha);
/* 0..255 stable spatial density, limited to this arc and reset on return. */
void dtr_arc_density(int cx,int cy,float inner,float outer,int first,int last,int r,int g,int b,int alpha,int density);
/* Pattern: 0 uniform, 1 inside-out density, 2 left-to-right density. */
void dtr_arc_reveal(int cx,int cy,float inner,float outer,int first,int last,int r,int g,int b,int alpha,int density,int pattern);
void dtr_text(const char *s,int x,int y,int size,int r,int g,int b,int alpha,int centered);
void dtr_metal_ring(int cx,int cy,int radius,int thickness,float (*profile)(float));
struct dtr_metal_texel { int8_t x,y; uint8_t grey,alpha; };
extern uint32_t dtr_profile_cycles[3]; /* ring / arcs / text, diagnostics only */
